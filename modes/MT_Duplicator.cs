...
if(!$HilightColor)
	for(%best = 1; %i < 64 && %best != 0; %i++)
	{
		%val = getColorIdTable(%i);
		%val = vectorDist(%val, "1 1 1");
		if(%val > %best)
			continue;
		$HilightColor = %i;
		%best = %val;
...
package Duplicator
{
	function serverCmdDuplicator(%cl, %arg)
	{
		%pl = %cl.player;
		if(!isObject(%pl))
			return;
		...
		%cl.MultiTool = MT_Modes.idx_["Duplicator"];
	}
	...
	function dupRotate(%evnt, %dir, %dif)
	{
		echo(%evnt," | ",%dir," | ",%dif);
		if(%evnt !$= "")
		{
